...
- [ ] **Step 1: Extend the schema mapping test**
...
- [ ] **Step 2: Run the mapping test and confirm the new entity is missing**

Run: `npm run test --workspace @workspace/api -- database-schema-mapping.spec.ts --runInBand`

Expected: FAIL because `TaskActivity` and `Task.archivedAt` do not exist.
...
**Interfaces:**
- Produces: controlled components whose props contain values, pending/error flags, and callbacks; components do not call APIs directly.
- Preserves: all current labels and accessible names used by `TaskBoardPage.test.tsx`.

- [ ] **Step 1: Add a structural regression assertion**

Extend the existing page test to assert the editor, AI planner, and task card remain available together after loading a board and opening a task. Keep the current creation, editing, status and AI tests unchanged.

- [ ] **Step 2: Run the current page suite as a green baseline**

Run: `npm run test --workspace @workspace/web -- TaskBoardPage.test.tsx --run`

Expected: all existing tests plus the structural assertion pass.

- [ ] **Step 3: Extract controlled components without changing data flow**
...
`AiTaskPlanner` receives `goal: string`, `drafts: AiTaskDraft[]`, the two pending flags, error text, `onGoalChange(value)`, `onDraftChange(index, draft)`, `onDraftRemove(index)`, `onGenerate()` and `onConfirm()` callbacks. Keep API requests and page state in `TaskBoardPage` for now.

- [ ] **Step 4: Run page tests and web build**

Run: `npm run test --workspace @workspace/web -- TaskBoardPage.test.tsx --run`

Expected: PASS with the same user-visible behavior.
...
```bash
git add apps/web/src/components/tasks apps/web/src/pages/TaskBoardPage.tsx apps/web/src/pages/TaskBoardPage.test.tsx
git commit -m "拆分任务看板组件"
```

### Task 6: 前端 URL 筛选、截止日期和截止标签
...
- [ ] **Step 4: Implement URL-backed filters and resilient loading**
...
- [ ] **Step 5: Add creation date and card labels**
...
- [ ] **Step 1: Write failing seed safety and idempotency-shape tests**

Mock `client.query` and assert the seed opens one transaction, uses fixed UUID parameters, uses `ON CONFLICT` for users/members/projects/tasks/activities, commits on success, and rolls back on a simulated query failure. Test the entry guard separately:
...
Expected: FAIL because seed functions do not exist.

- [ ] **Step 3: Implement fixed, non-destructive Demo data**

Use constants for two users, one team, memberships, one project, representative active/archived tasks and matching activities. Use parameterized queries and `ON CONFLICT (id) DO UPDATE`; never delete rows outside the fixed IDs. Hash `DEMO_USER_PASSWORD` with bcrypt cost 12 before passing it into the transaction.
...
- [ ] **Step 4: Add the command and example environment variable**

Add `"db:seed-demo": "ts-node src/database/seed-demo.ts"` and `DEMO_USER_PASSWORD=`. The entrypoint connects once, calls `seedDemoData`, prints only Demo emails and project name, and closes in `finally`; it never prints the password or connection URL.
...
```bash
git add apps/api/src/database/demo-seed.ts apps/api/src/database/demo-seed.spec.ts apps/api/src/database/seed-demo.ts apps/api/package.json apps/api/.env.example
git commit -m "添加幂等 Demo 数据脚本"
```

### Task 9: README、演示脚本、截图和最终验证
...
**Interfaces:**
- Consumes: Tasks 1-8 的 migration、seed、UI 和测试命令。
...
Create a specifically named disposable PostgreSQL database, apply `schema.sql`, run migrations, run seed twice, and query the fixed IDs to prove counts do not grow. Resolve the database name to an explicit value before cleanup; drop only that named disposable database after verification.
